// attempt to import node_yaml functions
...
function getComment(url, request) {
  ...
  if (comment) {
    // comment.comments = article.commentIds.map(
    //  commentId => database.comments[commentId]);
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    // remove comment id from author's comment ids
...
// if the following lines (through 410) are commented out, this project works perfectly and passes all tests.
// leaving them in causes errors. My implementation of YAML saving/loading isn't quite right.

function loadDatabase () {
  const dataFile = './database.yaml';
  // trying to check for existence of the save file, but it seems to always exist. Presuming assignment here is incorrect.
  if (dataFile) {
    Yaml.read(dataFile);
    console.log('database loaded - get back to it!');
  } else {
    console.log('error - database did not load');
  };
}
...
function saveDatabase () {
  ...
  if (dataFile) {
    console.log('database saved - booyah!')
  } else {
    console.log('error - database did not save');
  }
}
